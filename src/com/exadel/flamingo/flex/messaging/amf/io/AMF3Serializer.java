...
public class AMF3Serializer extends DataOutputStream implements ObjectOutput, AMF3Constants {
    ...
    public AMF3Serializer(OutputStream out) {
        ...
        log.fine("new AMF3Serializer(out="+ out+ ")");
    }
    ...
    protected void writeAMF3Integer(int i) throws IOException {
    	log.fine("writeAMF3Integer(i="+ String.valueOf(i)+ ")");

    	if (i < AMF3_INTEGER_MIN || i > AMF3_INTEGER_MAX) {
    		log.fine("writeAMF3Integer() - "+ Integer.valueOf(i)+ " is out of AMF3 int range, writing it as a Number");
        writeAMF3Number(i);
    	} else {
        write(AMF3_INTEGER);
    ...
    protected void writeAMF3IntegerData(int i) throws IOException {
    	log.fine("writeAMF3IntegerData(i="+ String.valueOf(i)+ ")");
    ...
    protected void writeAMF3Number(double d) throws IOException {
    	log.fine("writeAMF3Number(d="+ Double.valueOf(d)+ ")");
    ...
    protected void writeAMF3String(String s) throws IOException {
    	log.fine("writeAMF3String(s="+ StringUtil.toString(s)+ ")");
    ...
    protected void writeAMF3StringData(String s) throws IOException {
    	log.fine("writeAMF3StringData(s="+ StringUtil.toString(s)+ ")");
    ...
    protected void writeAMF3Date(Date date) throws IOException {
    	log.fine("writeAMF3Date(date="+ date+ ")");
    ...
    protected void writeAMF3Array(Object array) throws IOException {
    	log.fine("writeAMF3Array(array="+ array+ ")");
    ...
    protected void writeAMF3ByteArray(byte[] bytes) throws IOException {
    	log.fine("writeAMF3ByteArray(bytes="+ bytes+ ")");
    ...
    protected void writeAMF3Collection(Collection<?> c) throws IOException {
    	log.fine("writeAMF3Collection(c="+ c+ ")");
    ...
    protected void writeAMF3Object(Object o) throws IOException {
      ...
      if (index >= 0)
          ...
          log.fine("writeAMF3Object() - classGetter="+ classGetter);

          Class<?> oClass = classGetter.getClass(o);
          log.fine("writeAMF3Object() - oClass="+ oClass);
          ...
          log.fine("writeAMF3Object() - desc="+ desc);
          ...
          if (desc.isExternalizable()) {
              ...
              if (externalizer != null) {
                log.fine("writeAMF3Object() - using externalizer="+ externalizer);
                try {
                ...
                log.fine("writeAMF3Object() - legacy Externalizable="+ o);
                ((Externalizable)o).writeExternal(this);
              }
          }
          else {
            log.fine("writeAMF3Object() - writing defined properties...");
              for (int i = 0; i < desc.getPropertiesCount(); i++) {
                Object obj = desc.getPropertyValue(i, o);
                log.fine("writeAMF3Object() - writing defined property: "+ desc.getPropertyName(i) + "=" + obj);
                writeObject(obj);
              }

              if (desc.isDynamic()) {
                log.fine("writeAMF3Object() - writing dynamic properties...");
                Map<?, ?> oMap = (Map<?, ?>)o;
                for (Map.Entry<?, ?> entry : oMap.entrySet()) {
                    ...
                    if (key != null) {
                        ...
                        if (propertyName.length() > 0) {
                          log.fine("writeAMF3Object() - writing dynamic property: "+ propertyName + "=" + entry.getValue());
        ...
        log.fine("writeAMF3Object(o=" + o + ") - Done.");
    }
    ...
    protected void addToStoredStrings(String s) {
        if (!storedStrings.containsKey(s)) {
        	...
        	log.fine("addToStoredStrings(s=" + StringUtil.toString(s) + ") at index=" + index.toString());
    ...
    protected int indexOfStoredStrings(String s) {
        ...
        log.fine("indexOfStoredStrings(s=" + StringUtil.toString(s) + ") -> " + String.valueOf((index != null ? index.intValue() : -1)));
    ...
    protected void addToStoredObjects(Object o) {
        if (o != null && !storedObjects.containsKey(o)) {
        	...
        	log.fine("addToStoredObjects(o="+ o+ ") at index=" + index.toString());
    ...
    protected int indexOfStoredObjects(Object o) {
        ...
        log.fine("indexOfStoredObjects(o=" + o + ") -> " + String.valueOf((index != null ? index.intValue() : -1)));
        return (index != null ? index.intValue() : -1);
    }
    ...
    protected IndexedJavaClassDescriptor addToStoredClassDescriptors(Class<?> clazz) {
        ...
        log.fine("addToStoredClassDescriptors(clazz="+ clazz + ")");
...
*/
        ...
        log.fine("addToStoredClassDescriptors() - putting: name="+ name+ ", iDesc="+ iDesc);
